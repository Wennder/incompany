...
foreach(array_keys($this->associations) as $type): if($recursion < 0 and ($type != "belongsTo" && $recursion <= 0)) continue;

foreach($this->{$type} as $name => $association): foreach($results as $key => $result): $name = Inflector::underscore($name); $model = $association["className"]; $params = array(); if($type == "belongsTo"): $params["conditions"] = array( $association["foreignKey"] => $result[$this->primaryKey] ); $params["recursion"] = $recursion - 1;

elseif($type == "hasOne"): $params["conditions"] = array_merge( $association["conditions"], array( $association["primaryKey"] => $result[$association["foreignKey"]] ) ); else: $params["conditions"] = array_merge( $association["conditions"], array( $association["foreignKey"] => $result[$association["primaryKey"]] ) );

$params["recursion"] = $recursion - 2; endif; $result = $this->{$model}->all($params); if($type != "hasMany" && !empty($result)): $result = $result[0]; endif; $results[$key][$name] = $result; endforeach; endforeach; endforeach; return $results; }
    /**
     ...
     *  @return array Resultados da busca
     */
    public function toList($params = array(),$optionalText=true) {
        $params = array_merge(
            array(
                "key" => $this->primaryKey,
    ...
    public function save($data,$returnID=false) {
        ...
        if(isset($this->schema["who"])){
            $data["who"] = $by["id"]." - ".$by["nome"];
        }
        if(isset($this->schema["owner"])){
            $data["owner"] = $by["empresa_id"];
        }
