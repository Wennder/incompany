...
function PDFTable($objeto=null,$orientation='P',$unit='mm',$format='A4'){
	parent::FPDF($orientation,$unit,$format);
	$this->SetMargins(20,20,20);
	$this->SetAuthor('Autor do e-Syntex: Robson Miranda Alves de Oliveira');
	$this->_makePageSize();
...
function _tableWrite(&$table){
	...
	if ($table['nobreak'] && $table['tth']+$y0>$this->bottom && $table['multipage']){
		$this->AddPage($this->CurOrientation);
		$table['lasty'] = $this->y;
	}else
		$table['lasty'] = $y0;
